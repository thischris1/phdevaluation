...
xa = np.linspace(-5, 5, endpoint=True, num=200)
ya = np.linspace(-5, 5, endpoint=True, num=200)
...
fourierTransform = np.fft.fftshift(np.fft.fft2(z))
...
#plt.show()
surf2 = ax2.plot_surface(fourierSpacex, fourierSpacey, np.log(np.abs(fourierTransform)), cmap=cm.coolwarm, linewidth=0, antialiased=True, rstride=2, cstride=2)
