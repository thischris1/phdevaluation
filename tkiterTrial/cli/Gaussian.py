...
def procedure(i, impCount, Vmax,sigma, returnPotential=False):
    ...
    # run gaussian -p
    os.system("gaussian -p 2>/dev/null")
...
def calculatePotentialVarianceFromFile(fileName):
...
def readAndParseImpurity(fileName):
    impData = np.loadtxt(fileName)
    impCount = len(impData)
    sigmax = impData[0,3]
    vmax = np.max(impData[:,4])
    vmin = np.min(impData[:,4])
    return impCount, sigmax, vmin, vmax
